...
export default class TokenContractBenchmark extends Benchmark {
  ...
  getMethods(context: TokenBenchmarkContext): ContractFunctionInteractionCallIntent[] {
    ...
    const methods: ContractFunctionInteractionCallIntent[] = [
      ...
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.withdraw_public_to_public(bob, alice, amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.withdraw_public_to_private(bob, alice, amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.withdraw_private_to_private(bob, alice, amt(1), amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.withdraw_private_to_public_exact(bob, alice, amt(1), amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.withdraw_private_to_private_exact(bob, alice, amt(1), amt(1)),
      },

      // Redeem methods
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.redeem_public_to_public(bob, alice, amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.redeem_private_to_public(bob, alice, amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.redeem_private_to_private_exact(bob, alice, amt(1), amt(1)),
      },
      {
        caller: bob,
        action: vaultContract.withWallet(wallet).methods.redeem_public_to_private_exact(bob, alice, amt(1), amt(1)),
      },
    ];
